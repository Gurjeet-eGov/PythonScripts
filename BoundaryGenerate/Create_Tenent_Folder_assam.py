...
def create_json_bound(state_code,city_code,city_name, block_code, block_name):
    
    bdata = {
              "tenantId": state_code+"."+city_name.lower().replace(" ", ""),
              ...
              "TenantBoundary": [
                {
                  ...
                  "boundary": {
                    ...
                    "localname": city_name.upper(),
                    "longitude": None,
                    "latitude": None,
                    "label": "City",
                    "code": state_code+"."+city_name.lower().replace(" ", ""),
                    "children":[
                      {
                        "id": 1,
                        "boundaryNum": 1,
                        "name": block_name,
                        "localname": block_name,
                        "longitude": None,
                        "latitude": None,
                        "label": "Locality",
                        "code": block_code
    ...
    filename = "data/"+state_code+"/"+city_code.lower().replace(" ", "")+"/egov-location/boundary-data.json"
    os.makedirs(os.path.dirname(filename), exist_ok=True)
...
df = pd.read_excel("KA_data_IMIS2.xlsx",dtype=str)
display(df)
...
for index, row in df.iterrows():
    ...
    city_folder = row['unique_tenant_code']
    city_folder_local_name = row['tenant_name']
    child_block_code = row['blockcode']
    child_block_name = row['blockname']

    # create_json_billing(state_code,pb_village_folder)
    create_json_bound(state_code, city_folder, city_folder_local_name, child_block_code, child_block_name)
