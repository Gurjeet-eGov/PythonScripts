...
def NewTL_mod(epoch_key, epoch_value):
    ...
    fetch_bill_response = TL_obj.fetch_bill(csr_login_RequestInfo, Application_ID, "TL")
    collection_response = TL_obj.collect_fee(csr_login_RequestInfo, fetch_bill_response)


    search_param = {"tenantId": TL_obj.CITY, "applicationNumber": Application_ID}
    tl_search_response = TL_obj.call_api(csr_login_RequestInfo, payload={}, 
...
def NewTL_mod_tenant(epoch_key, epoch_value, tenant):
    ...
    # CSR Create
    csr_create_response = TL_obj.call_api(su_login_RequestInfo, "TL/csr_create_payload.json", TL_obj.get_endpoint("create"))["Licenses"]
    ...
    csr_update_response = TL_obj.call_api(su_login_RequestInfo, csr_update_data, TL_obj.get_endpoint("update"))["Licenses"]


    su_login_RequestInfo = TL_obj.login("FieldEmp")
    # DV Update
    ...
    dv_update_response = TL_obj.call_api(su_login_RequestInfo, dv_update_data, TL_obj.get_endpoint("update"))["Licenses"]
    ...
    fi_update_response = TL_obj.call_api(su_login_RequestInfo, fi_update_data, TL_obj.get_endpoint("update"))["Licenses"]
    # logout FieldEmp
    TL_obj.logout(su_login_RequestInfo)

    
    su_login_RequestInfo = TL_obj.login("OfficeEmp")
    # AP Update payload
    ...
    fetch_bill_response = TL_obj.fetch_bill(su_login_RequestInfo, Application_ID, "TL")
    collection_response = TL_obj.collect_fee(su_login_RequestInfo, fetch_bill_response)
