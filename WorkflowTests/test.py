...
class TEST:
    ...
    def __init__(self):
        env_conf = utils.read_json("env_configs.json")
        self.BASE_URL = env_conf["BASE_URL"]
        self.HEADERS = env_conf["HEADERS"]
        self.ENDPOINTS = env_conf["ENDPOINTS"]
        self.STATE = env_conf["STATE"]
        self.CITY = env_conf["CITY"]
        self.LOGIN_HEADER = env_conf["LOGIN_HEADER"]

    def get_endpoint(self, api_name):
        with open(self.ENDPOINTS) as file: 
    ...
    def login(self, username, password, tenantId, userType):
        ...
        response = requests.post(url = self.BASE_URL + self.get_endpoint("login"), 
    ...
    def NewTL():
        ...
        TL_Endpoints = get_endpoint("TL")
        csr_create_response = call_api(csr_login_RequestInfo, "TL/csr_create_payload.json", TL_Endpoints["createTL"])["Licenses"]
        ...
        # CSR Update
        csr_update_response = call_api(csr_login_RequestInfo, csr_update_data, TL_Endpoints["updateTL"])["Licenses"]
        ...
        # DV Update
        dv_update_response = call_api(login_RequestInfo, dv_update_data, TL_Endpoints["updateTL"])["Licenses"]
        ...
        # FI Update
        fi_update_response = call_api(login_RequestInfo, fi_update_data, TL_Endpoints["updateTL"])["Licenses"]
        ...
        # AP Update
        ap_update_response = call_api(login_RequestInfo, ap_update_data, TL_Endpoints["updateTL"])["Licenses"]
